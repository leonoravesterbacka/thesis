...
%Despite its success, there are phenomena that the SM is unable to explain, such as the origin of dark matter and dark energy. 
...
For each of the fermions there is a corresponding antiparticle, doubling the population of the particle zoo. 
...
Groups enter in to the formulation of particle physics as they describe the property of carrying out a transformation on a physical system that leaves the system unchanged. 
...
Ignoring the mass term for a second, the Lagrangian can now be written as
\begin{equation}
...
%As two quarks separate, there is a force acting between them that increase until the point where it is energetically more favorable for it to split the energy into two quark anti-quark pairs in between the two original ones. This recovers the zero color charge charge, and leads to the experimental effect that quarks can never be measured separately. 
...
\begin{figure}[htbp!]
\begin{center}
    \includegraphics[width=0.44\textwidth]{images/theory/higgspotential.png}
...
\end{center}
\end{figure}
Spontaneous symmetry breaking occurs when one of the infinite possibilities of minima is chosen, similarly to how a bead would roll down the slope of the sombrero and settle in one of the positions around the brim of the hat.  
...
\end{equation}
and the values are listed in \cite{PhysRevD.98.030001}
\begin{equation}
...
%\noindent\justify
%The SM Lagrangian summarized in the previous sections can be used to calculate the scattering probabilities that are the basis from which cross sections and branching fractions are calculated.
%The simplest calculation is the so called leading order (LO) calculation, where one assumes the simplest scattering process, such as the one visualized in Figure \ref{fig:fLO}
%\begin{figure}[!htb]
%\begin{center}
...
%\fmf{fermion}{o1,v2}
%\fmf{fermion}{v2,o2}
%\fmf{photon,label=$\phi$}{v1,v2}
%\end{fmfgraph*}
%\end{fmffile}
%\end{tabular}
%\end{center}
%\caption{Leading order process.}
%\label{fig:fLO}
%\end{figure}   
%Here each interaction vertex is  
...
\newpara
\noindent\justify
%feature of the SM that was glanced over in the introduction, is the vast ranges that the masses of SM particles span over. 
